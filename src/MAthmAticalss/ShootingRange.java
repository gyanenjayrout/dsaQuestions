...
/* 
 * 
 * You are at a shooting range. You have N targets labeled 1 to N from left to right. Hit the target
 *  by performing the following operation:. In each round starting from the first target and moving from left to right, 
 *  hit every alternate target.. Repeat the above step until only one target is left Note: You will get i points when you hit
 *   a target labeled i. calculate the score you will obtain by 
 * continuously hitting targets until there is only one target is remaining write the optimized code in java. dry run an example
 * 
 * Observing the Pattern:
 * 👉 If n = 5 (Targets: [1, 2, 3, 4, 5])

    Round 1: Hit every alternate → Remaining: [2, 4]
    Round 2: Hit every alternate → Remaining: [4]
    ✅ Answer = 4

👉 If n = 6 (Targets: [1, 2, 3, 4, 5, 6])

    Round 1: Hit every alternate → Remaining: [2, 4, 6]
    Round 2: Hit every alternate → Remaining: [4]
    ✅ Answer = 4

👉 If n = 9 (Targets: [1, 2, 3, 4, 5, 6, 7, 8, 9])

    Round 1: Hit every alternate → Remaining: [2, 4, 6, 8]
    Round 2: Hit every alternate → Remaining: [4, 8]
    Round 3: Hit every alternate → Remaining: [8]
    ✅ Answer = 8
    📊 Pattern Insight:

After simulating multiple cases, the final target is always the largest power of 2 ≤ n.
Why is that?

    Each round halves the list by keeping even-indexed elements.
    Powers of 2 (like 1, 2, 4, 8, 16, etc.) always survive the longest.
    The largest power of 2 is the last "survivor" because it is perfectly aligned with the elimination process.
 */
public class ShootingRange {
	...
	public static int calculateScore(int n) {
        ...
        int powerOfTwo = 1;
        while (powerOfTwo <= n) {
            powerOfTwo <<= 1; // Multiply by 2  -> Left shift by 1 (x << 1) is the same as multiplying by 2.
        }
        powerOfTwo >>= 1; // Get the last valid power of 2 -> Go back to the last valid power of 2
